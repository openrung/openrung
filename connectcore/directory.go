...
type relayFetcher func(ctx context.Context, brokerURL string, opts discovery.Options) (brokerapi.RelayListResponse, error)
...
type directoryCache struct {
	...
	mu        sync.Mutex
	cached    *brokerapi.RelayListResponse
...
func newDirectoryCache() *directoryCache {
	return &directoryCache{
		fetcher: func(ctx context.Context, brokerURL string, opts discovery.Options) (brokerapi.RelayListResponse, error) {
...
func (s *Engine) ListRelaysForDirectory() (brokerapi.RelayListResponse, error) {
...
func (d *directoryCache) fetch(ctx context.Context, brokerURL string, opts discovery.Options) (brokerapi.RelayListResponse, error) {
	d.flightMu.Lock()
...
func (d *directoryCache) snapshotFor(brokerURL string, withinInterval bool) (brokerapi.RelayListResponse, bool) {
	d.mu.Lock()
	defer d.mu.Unlock()
	now := d.clock()
	if d.cachedURL != brokerURL || !directorySnapshotFresh(d.cached, now) {
		return brokerapi.RelayListResponse{}, false
	}
...
func directorySnapshotFresh(response *brokerapi.RelayListResponse, now time.Time) bool {
	return response != nil &&
