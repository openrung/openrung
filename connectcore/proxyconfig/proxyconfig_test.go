...
// dirStore is a minimal file-backed PortStore+HelperStore standing in for the
// root module's clientstate.Store, which this module cannot import (the real
// store's interface satisfaction is asserted in internal/clientstate). Like
// the real store it persists under one directory, so a second store over the
// same directory sees the first one's port, and a directory that cannot be
// created surfaces as a persistence error rather than a panic.
type dirStore struct{ dir string }

func newDirStore(dir string) dirStore { return dirStore{dir: dir} }

func (s dirStore) portPath() string { return filepath.Join(s.dir, "proxy-port") }

func (s dirStore) SaveProxyPort(port int) error {
	if err := os.MkdirAll(s.dir, 0o700); err != nil {
		return err
	}
	return os.WriteFile(s.portPath(), []byte(strconv.Itoa(port)), 0o600)
}

func (s dirStore) LoadProxyPort() (int, bool) {
	data, err := os.ReadFile(s.portPath())
	if err != nil {
		return 0, false
	}
...
func (s dirStore) LoadOrSaveProxyPort(candidate int) (int, error) {
...
func (s dirStore) SaveProxyEnvScript(_ int, script []byte) (string, error) {
	if err := os.MkdirAll(s.dir, 0o700); err != nil {
		return "", err
	}
	path := filepath.Join(s.dir, "proxy-env.sh")
...
func TestResolvePortReusesOrCreatesPersistedPort(t *testing.T) {
	...
	second, err := resolvePort(newDirStore(dir), lookup, func() (int, error) {
...
func TestResolvePortKeepsWorkingWhenPersistenceFails(t *testing.T) {
	...
	store := newDirStore(filepath.Join(blocker, "openrung"))
...
func TestWriteShellHelperQuotesPathAndBuildsEndpoint(t *testing.T) {
	...
	info, err := WriteShellHelper(newDirStore(dir), 46685)
	if err != nil {
		t.Fatal(err)
	}
