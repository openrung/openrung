...
func unranked(cands []brokerapi.RelayDescriptor) []rankedRelay {
...
func rankByTCPLatency(
	...
	head, tail := cands, []brokerapi.RelayDescriptor(nil)
...
func (s *Engine) rankLadder(ctx context.Context, cands []brokerapi.RelayDescriptor, target RelayTarget) ladderOrder {
...
type ladderOrder struct {
	brokerOrder []brokerapi.RelayDescriptor // post-filter, pre-ranking, as the broker served it
	ranked      []rankedRelay               // ladder order; probeMS nil when unprobed
}

// candidates is the ladder order as plain descriptors.
func (o ladderOrder) candidates() []brokerapi.RelayDescriptor {
	out := make([]brokerapi.RelayDescriptor, 0, len(o.ranked))
