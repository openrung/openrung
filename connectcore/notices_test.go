...
func TestWSSTicketRetryEmitsTypedNotice(t *testing.T) {
	...
	s.requestWSSTicket = func(context.Context, string, brokerapi.WSSTicketRequest, string, string) (brokerapi.WSSTicketResponse, error) {
		if int(atomic.AddInt32(&calls, 1)) <= len(fronts) {
			return brokerapi.WSSTicketResponse{}, &client.WSSTicketStatusError{StatusCode: 429, RetryAfter: 2 * time.Second}
		}
	...
	conn := &connection{}
	ticket, err := s.requestWSSSessionTicket(context.Background(), conn, brokerapi.WSSTicketRequest{
		RelayID: "relay-a",
		FrontID: front.ID,
	})
...
func TestPunchFailureEmitsOutcomeNotice(t *testing.T) {
	...
	var notices []Notice
	// The establisher fails the hub coordination call, so the attempt reports
	// its outcome without any network wait. The fake also pins that the
	// BaseURL override reaches the hub client unchanged.
	establish := func(_ context.Context, hub punchcore.HubClient, _ string) (*PunchPath, punchcore.PunchResult, error) {
		if hub.BaseURL != "http://127.0.0.1:1" {
			t.Fatalf("hub base URL = %q, want the -punch-url override", hub.BaseURL)
		}
		return nil, punchcore.PunchResult{Reason: "config"}, errors.New("hub coordination refused")
	}
	est := AttemptPunch(ctx, nil, brokerapi.RelayDescriptor{ID: "relay-a", PunchCapable: true}, PunchOptions{
		Enabled:   true,
