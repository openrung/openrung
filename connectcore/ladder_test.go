...
func relayAt(id, countryCode, city, country, host string) brokerapi.RelayDescriptor {
...
// machine.
func newLadderService(t *testing.T, relays func() []brokerapi.RelayDescriptor) (*Engine, *testSink) {
	t.Helper()
	tmp := t.TempDir()
	t.Setenv("HOME", tmp)
...
func TestDisconnectMidLadderIsNotAFailure(t *testing.T) {
	sink := newTelemetrySink(t)
	fixtures := []brokerapi.RelayDescriptor{relayAt("a", "JP", "", "Japan", "127.0.0.10")}
	s, _ := newLadderService(t, func() []brokerapi.RelayDescriptor { return fixtures })
...
func TestConnectTargetLabelPinsLabelledRelays(t *testing.T) {
	...
	sink := newTelemetrySink(t)
	labelled := func(id, host, label string) brokerapi.RelayDescriptor {
...
func TestActiveConnectionInfoTracksPromotedCandidate(t *testing.T) {
	...
	if info.Relay.ID != "a" || info.Transport != brokerapi.TransportDirect || info.FrontID != "" {
