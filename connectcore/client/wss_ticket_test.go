...
func TestBrokerClientRequestWSSSessionTicket(t *testing.T) {
	...
	wantResponse := brokerapi.WSSTicketResponse{
	...
	httpClient := &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		...
		var gotRequest brokerapi.WSSTicketRequest
	...
	got, err := client.RequestWSSSessionTicket(t.Context(), brokerapi.WSSTicketRequest{
		RelayID: "relay-1",
		FrontID: "gateway-1",
	}, "client-1", "session-1")
	if err != nil {
...
func TestBrokerClientRequestWSSSessionTicketOmitsIncompleteIdentity(t *testing.T) {
	...
	httpClient := &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		...
		body, _ := json.Marshal(brokerapi.WSSTicketResponse{
	...
	_, err := (BrokerClient{BaseURL: "https://broker.example.com", HTTPClient: httpClient}).RequestWSSSessionTicket(
		t.Context(), brokerapi.WSSTicketRequest{RelayID: "relay-1", FrontID: "gateway-1"}, "client-only", "",
	)
	if err != nil {
		t.Fatalf("request WSS ticket: %v", err)
	}
...
func TestBrokerClientRequestWSSSessionTicketValidatesRequestBeforeHTTP(t *testing.T) {
	...
	for _, request := range []brokerapi.WSSTicketRequest{
		{FrontID: "gateway-1"},
		{RelayID: "relay-1"},
	} {
...
func TestBrokerClientRequestWSSTicketStatusErrorIgnoresOriginBody(t *testing.T) {
	...
	_, err := (BrokerClient{BaseURL: "https://broker.example.com", HTTPClient: httpClient}).RequestWSSSessionTicket(
		t.Context(), brokerapi.WSSTicketRequest{RelayID: "relay-1", FrontID: "gateway-1"}, "", "",
	)
...
func TestBrokerClientRequestWSSSessionTicketRejectsOversizedResponse(t *testing.T) {
	...
	_, err := (BrokerClient{BaseURL: "https://broker.example.com", HTTPClient: httpClient}).RequestWSSSessionTicket(
		context.Background(), brokerapi.WSSTicketRequest{RelayID: "relay-1", FrontID: "gateway-1"}, "", "",
	)
...
func TestBrokerClientRequestWSSSessionTicketRejectsRedirectWithoutLeakingIdentity(t *testing.T) {
	...
	_, err := (BrokerClient{BaseURL: "https://broker.example.com", HTTPClient: httpClient}).RequestWSSSessionTicket(
		t.Context(), brokerapi.WSSTicketRequest{RelayID: "relay-1", FrontID: "gateway-1"}, "client-1", "session-1",
	)
	var statusErr *WSSTicketStatusError
