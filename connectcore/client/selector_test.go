...
func TestSelectRelaySelectsFirstUsableRelay(t *testing.T) {
	...
	selected, err := SelectRelay(brokerapi.RelayListResponse{
		ServerTime: now,
		Relays:     []brokerapi.RelayDescriptor{wrongProtocol, second},
	})
	if err != nil {
...
func TestSelectRelaySkipsExpiredRelay(t *testing.T) {
	...
	_, err := SelectRelay(brokerapi.RelayListResponse{
		ServerTime: now,
		Relays:     []brokerapi.RelayDescriptor{expired},
	})
...
func TestSelectRelaySkipsIncompleteRelay(t *testing.T) {
	...
	_, err := SelectRelay(brokerapi.RelayListResponse{
		ServerTime: now,
		Relays:     []brokerapi.RelayDescriptor{incomplete},
	})
	if !errors.Is(err, ErrNoUsableRelay) {
		t.Fatalf("expected ErrNoUsableRelay, got %v", err)
	}
...
func TestIsUsableRelayRequiresDirectVLESSRealityVision(t *testing.T) {
	...
	candidate.ExitMode = brokerapi.ExitModeDedicated
...
func TestSelectRelayAutoPreservesBrokerOrder(t *testing.T) {
	...
	selected, err := SelectRelay(brokerapi.RelayListResponse{
		ServerTime: now,
		Relays:     []brokerapi.RelayDescriptor{ipv4, ipv6},
	})
	if err != nil {
		t.Fatalf("select relay: %v", err)
	}
...
func TestSelectRelayForFamilyCanPreferIPv4Relay(t *testing.T) {
	...
	selected, err := SelectRelayForFamily(brokerapi.RelayListResponse{
		ServerTime: now,
		Relays:     []brokerapi.RelayDescriptor{ipv6, ipv4},
	}, RelayFamilyIPv4)
	if err != nil {
		t.Fatalf("select relay: %v", err)
	}
...
func TestSelectRelayForFamilyRequiresMatchingFamily(t *testing.T) {
	...
	_, err := SelectRelayForFamily(brokerapi.RelayListResponse{
		ServerTime: now,
		Relays:     []brokerapi.RelayDescriptor{ipv6},
	}, RelayFamilyIPv4)
	if !errors.Is(err, ErrNoUsableRelay) {
		t.Fatalf("expected ErrNoUsableRelay, got %v", err)
	}
...
func validRelay(now time.Time) brokerapi.RelayDescriptor {
