...
// click action), copied from desktop/frontend/src/components/SettingRow.tsx.
// Renders as a button when onPress is given, otherwise a static panel.
...
export function SettingRow({ title, subtitle, trailing, onPress }: Props) {
  const inner = (
    <>
      ...
      {trailing}
      {onPress != null && trailing == null && (
        <span className="or-setting-chevron">{'\u25B8'}</span>
      )}
    </>
  );
