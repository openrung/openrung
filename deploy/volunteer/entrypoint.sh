...
# The registration token (OPENRUNG_VOLUNTEER_TOKEN), label (OPENRUNG_LABEL), and
# node class (OPENRUNG_NODE_CLASS) are read natively from the environment by the
# binary, so they need no flag mapping.
